    ...
    def detect_format(text):
        ...
        try:
            ...
            if isinstance(json_data, dict) and all(
                isinstance(v, str) for v in json_data.values()
            ):
...
class ModDictionary:
    def __init__(self, mod_namespace):
...
class MainWindow(QMainWindow):
    ...
    def setup_editor_tab(self):
        ...
        # 设置分割比例
        splitter.setSizes([430, 780])  # 设置左侧列表宽度为430像素
    ...
    def open_translated_file(self):
        ...
        if file_name:
            try:
                ...
                # 更新UI
                self.translated_status.setText(
                    f"翻译文件: {os.path.basename(file_name)}"
                )

                # 计算已翻译条目数
                translated_count = sum(
                    1
                    for element in self.mod_dictionary.text_dictionary.values()
    ...
    def clear_duplicate_translations(self):
        ...
        for key, element in self.mod_dictionary.text_dictionary.items():
            if (
                element.is_translated
                and element.original_text == element.translated_text
            ):
                element.reset()
                count += 1
    ...
    def save_translation_file(self):
        ...
        options = QFileDialog.Options()
        file_format = (
            ".json" if self.mod_dictionary.format == LangFormat.JSON else ".lang"
        )
        file_name, _ = QFileDialog.getSaveFileName(
            self,
            "保存翻译文件",
            f"{self.mod_dictionary.mod_namespace}_zh_cn{file_format}",
...
if __name__ == "__main__":
    ...
    try:
        with open(
            os.path.join(os.path.dirname(__file__), "style.qss"), "r", encoding="utf-8"
        ) as f:
